...
HEIGHT = 256
WIDTH = 256
LR = 0.0004 # original:0.0002
EPOCHS = 20 # original: 30
BATCH_SIZE = 32
...
def visualize_depth_map(samples, test=False, model=None, name="noname"):
    input, target = samples
    cmap = plt.cm.jet
    cmap.set_bad(color="black")

    print("visualize_depth_map len: ", len(input), len(target))
    if test:
        pred = model.predict(input)
        fig, ax = plt.subplots(len(input), 3, figsize=(50, 50))
        ...
        fig, ax = plt.subplots(len(input), 2, figsize=(50, 50))
        for i in range(len(input)):
            ax[i, 0].imshow((input[i].squeeze()))
            ax[i, 1].imshow((target[i].squeeze()), cmap=cmap)
...
visualize_depth_map(visualize_samples, name="visualize_samples")
...
plt.savefig("3D_point_cloud_vis.png")
...
#model.save_model_image(x=np.random.randint(0,256, (256,256,4)))
...
visualize_depth_map(test_loader, test=True, model=model, name="output0")
